...
(function (exports) {
  ...
  exports.initialize = function (defaultEventHandlers, manageMode) {
    ...
    if (FrontendBook.manageMode) {
      ...
      if (
        selectedServiceId &&
        $selectService.find('option[value="' + selectedServiceId + '"]')
          .length > 0
      ) {
      ...
      if (
        selectedProviderId &&
        $selectProvider.find('option[value="' + selectedProviderId + '"]')
          .length === 0
      ) {
  ...
  exports.updateConfirmFrame = function () {
          ...
          html: [
            ...
            $("<span/>", {
              text:
                EALang.timezone +
                ": " +
                $("#select-timezone option:selected").text(),
            }),
            $("<br/>"),
            $("<span/>", {
    ...
    var appointment_price = 0.0;
    var eyes_value = "";
    if ($("input:radio[name=eyes]:checked").val() == "One Eye") {
      appointment_price = service.one_eye_price;
      eyes_value = "One Eye";
    } else {
      appointment_price = service.both_eyes_price;
      eyes_value = "Both Eyes";
    }
  ...
  function applyAppointmentData(appointment, provider, customer) {
    try {
      ...
      FrontendBookApi.getAvailableHours(
        moment(appointment.start_datetime).format("YYYY-MM-DD")
      );
  ...
  function updateServiceDescription(serviceId, final_price) {
    ...
    if (
      ...
      $("<span/>", {
        text:
          "[" + EALang.price + " " + final_price + " " + service.currency + "]",
      }).appendTo($serviceDescription);
    }
