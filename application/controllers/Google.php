...
class Google extends EA_Controller {
    ...
    /**
     * Complete synchronization of appointments between Google Calendar and 7keema.
     *
    ...
    public static function sync($provider_id = NULL)
    {
        try
        {
            ...
            foreach ($appointments as $appointment)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        {
                            throw new Exception('Event is cancelled, remove the record from 7keema.');
                        }

                        // If Google Calendar event is different from 7keema appointment then update
                        // 7keema record.
                        $is_different = FALSE;
            ...
            // Add Google Calendar events that do not exist in 7keema.
            ...
            foreach ($google_events->getItems() as $google_event)
            {
                ...
                // Record doesn't exist in the 7keema, so add the event now.
                $appointment = [
     ...
     * documentation of OAuth), every 7keema installation should use its own calendar api key. So in every
     * api console account, the "http://path-to-7keema/google/oauth_callback" should be included in an allowed redirect URL.
     */
